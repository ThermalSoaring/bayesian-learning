...
class CRRCSim(Aircraft):
    ...
    def recv_fdm(self):
        ...
        # Look at: crrcsim/mod_inputdev/inputdev_mnav/mnav.cpp
        ...
        hx, hy, hz,
        # Junk - all zeros
        char3, char4, char5, char6, char7, char8,
        # Pressure, m and m/s
        Ps, Pt ) = struct.unpack('>ccchhhhhhhhhcccccchh', buf[:31])

        if outputPacketType != 'N':
            print "Error: outputPacketType =", outputPacketType, "not 'N' as expected"
            return False
        ...
        # 'G'
        gpsPacketType,
        # GPS velocities (north, east, down) [m/s]
        vn, ve, vd,
        # GPS position [?]
        lon, lat, alt,
        # GPS time [?]
        gpsTime,
        # Junk - all zeros
        char11, char12, char13, char14,
        # Sum
        gpsSumHi, gpsSumLow
        ) = struct.unpack('<ccciiiiiihcccccc', buf[31:66])

        if gpsPacketType != 'G':
            print "Error: gpsPacketType =", gpsPacketType, "not 'G' as expected"
            return False
        ...
        # Sum
        totalSumHi, totalSumLo
        ) = struct.unpack('>cchhhhhhhhcc', buf[66:])

        if servoPacketType != 'F':
            print "Error: servoPacketType =", servoPacketType, "not 'F' as expected"
            return False
